...
=head1 SYNOPSIS
  ...
  logrd-redirect-streams file --copy-to-console stdlog
...
allowing writing to multiple streams.  For example,
  ...
  logrd-redirect-streams file1 --copy-to-console stdout

  echo "this goes to file1 and the console"

  logrd-redirect-streams file2 --copy-to-stream stdout
...
Multiple streams may be directed to a single file:
  ...
  logrd-redirect-streams file1 stdout stderr
...
to intermediate states is required, use subshells:
   ...
   (
     logrd-redirect-streams file2 stdout
     echo "this goes to file1, file2, and the console"
   )
...
stream, but may be redirected:
  ...
  logrd-redirect-streams file stdlog
...
existence when B<logrd> was loaded).  For example, this causes
  ...
  logrd-redirect-streams file stdout
  ls

to send the output of C<ls> to both I<file> and to the standard output stream

L</logrd-redirect-streams> will also accept this option to restrict its application
...
For example, this causes
  ...
  logrd-redirect-streams file1 stdout
  logrd-redirect-streams file2 stdout
  ls
...
existence when B<logrd> was loaded).  For example, this
  ...
  logrd-redirect-streams file --copy-to-console stdout stderr
...
Unknown log levels are rejected and leave the current level unchanged.
...
Global parameters may be initialized and later overridden from
environment variables. By default the variables have a prefix of C<LOGRD_> (note
the trailing underscore); that may be changed by the L</--env-prefix>
option when loading B<logrd>.

The environment is consulted when the library is sourced and again when
L</logrd-setup> runs.  If both matching environment variables and
L</logrd-setup> options are provided, the environment values take
precedence.
